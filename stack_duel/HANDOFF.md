...
- ~~**No audio**~~ **REVERSED in Build #6** (owner directive: match world
  standards). Audio is now in, via **self-synthesised WAV tones** (`assets/audio/`,
  no third-party assets → no licensing risk) through a `Sound` seam. The rising
  perfect tone is the genre signature.
...
- **Build #6 (current) — audio + particles.** Closed the biggest standard gap:
  sound. Self-synthesised WAV tones (`assets/audio/*.wav`, generated by a sine/
  triangle synth — zero third-party assets, zero licensing risk) via a `Sound`
  seam (`lib/game/sound.dart`, `flame_audio`): soft tick on a normal drop, a
  **rising tone keyed to combo** on perfect (the genre signature), a descending
  thud on game over. Plus a Flame-core particle burst on perfect. Tests: slice
  24/24, runtime **13/13** (sound wiring), analyze clean.
...
Mechanics (perfect/restore/combo) are verified by tests + match the proven
genre standard, so they need no per-build "feel" sign-off.

Strategic decision (owner: "decide yourself"): stay on Flutter/Flame (the only
