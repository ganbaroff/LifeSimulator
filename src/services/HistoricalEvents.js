// HistoricalEvents.js - База исторических событий 1850-2025
...
export const getHistoricalContext = (country, year) => {
  ...
  for (const eventYear in countryEvents) {
    const distance = Math.abs(parseInt(eventYear) - year);
...
export const applyHistoricalEffects = (baseEffects, historicalEvent) => {
  ...
  if (modified.wealth !== undefined) {
    modified.wealth = Math.floor(modified.wealth * historicalEvent.effects.wealth);
  }
  if (modified.happiness !== undefined) {
    modified.happiness = Math.floor(modified.happiness * historicalEvent.effects.happiness);
  }
...
export const generateHistoricalPromptContext = (country, year, age) => {
  ...
  return `It is ${year} in ${country}. Historical context: ${event.event} - ${event.description}. This significantly affects daily life, economy, and opportunities. The character is ${age} years old during this period.`;
};

// ФУНКЦИЯ: Проверка, является ли событие подходящим для текущего исторического контекста
export const isEventAppropriate = (eventTags, historicalEvent) => {
  if (!historicalEvent || !eventTags) return true;

  // Например, бизнес-события не подходят во время войны
  const warTimes = historicalEvent.tags.includes('war');
