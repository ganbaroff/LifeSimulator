# Stack Duel — Audit Request

> Published as a public file in the repo `ganbaroff/LifeSimulator` (a GitHub
> Gist could not be created from this environment — no `gh` CLI and no gist API
> available — so this is a public repo file instead; it opens on a phone without
> login). Below: the audit brief, then the complete current `stack_duel/HANDOFF.md`.

---

## AUDIT BRIEF

Проаудируй документ ниже (`stack_duel/HANDOFF.md`). Прочитай полностью с нуля,
затем дай разбор строго по этому брифу.

**КОНТЕКСТ**
Day 1 закрыт — луп проверен на устройстве (Android S24 Ultra) и затягивает
(Best 25). Цель v1 — выйти в Google Play и получить первый revenue-signal на
рекламе, а НЕ сделать великую игру. Не хвали. Только дельты, без пересказа
документа.

**ЧТО РАЗОБРАТЬ — ТОЛЬКО ЭТИ ЗОНЫ**
1. §10.2 Perfect / combo — дыры в балансе: бесконечная игра при идеальной
   точности, эксплойт комбо, читаемость.
2. §12 Monetization contract — навредит ли правило retention или eCPM
   (interstitial каждый 3-й game over; монеты только косметика;
   Perfect→combo→coins). Конкретно.
3. §14 exit-критерий Day 2 — верная финишная черта или чего-то не хватает, что
   ОБЯЗАНО войти в Day 2.
4. Срез / combo-математика — краевые случаи, которые 18 unit-тестов могут не
   ловить.

**ЖЁСТКИЕ ПРАВИЛА (нарушающая дельта отклоняется)**
- Никакого расширения скоупа. Новая фича обязана назвать, что ЗАМЕНЯЕТ, либо
  обоснована против тайм-бокса §14. По умолчанию reject.
- Не предлагай число для epsilon — подбирается под палец на устройстве.
- Не пересматривай решённое: Flutter+Flame, без бэкенда, изоляция в
  `stack_duel/`, звук/скины/IAP позже.
- Максимум 7 дельт. Приоритизируй.

**ФОРМАТ ОТВЕТА — нумерованно, каждая дельта:**
`{что} — {почему} — {что меняет} — {стоимость: S/M/L}`

**ДОПОЛНИТЕЛЬНО**
- Если в разделе (1–4) проблем нет — напиши «ОК», не пропускай молча.
- В конце пометь дельту №1 по важности.

---
---

## DOCUMENT UNDER AUDIT — `stack_duel/HANDOFF.md`
...
- Tune `_speedPerPoint` / `_maxSpeed` and the perfect-window epsilon together on
  device until the difficulty curve feels fair-but-tense.
- Camera easing: revisit `dt*6` lerp factor and `_topMargin` so fast stacks
  don't feel jumpy.

### Tier 3 — Light meta (only after Tier 1 feels great).
- Combo counter for consecutive perfects (score multiplier).
- Subtle background gradient that shifts with height/score.
